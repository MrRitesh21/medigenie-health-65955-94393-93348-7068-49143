...
export default function Records() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      } = await supabase.from("prescriptions").select("*").eq("patient_id", patientData.id).order("created_at", {
        ascending: false
      });
        ...
        <Tabs defaultValue="prescriptions" className="w-full">
          ...
          <TabsContent value="prescriptions" className="mt-6">
            {loading ? <div className="space-y-4">
              ...
              </div> : prescriptions.length === 0 ? <Card>
                ...
                {prescriptions.map(prescription => <Card key={prescription.id}>
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        {prescription.ai_generated && <Badge variant="secondary">
                            AI Generated
                          </Badge>}
                      </div>
                    </CardHeader>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="space-y-2">
                            {Array.isArray(prescription.medications) && prescription.medications.map((med: any, index: number) => <div key={index} className="p-3 bg-muted rounded-lg">
                                <p className="font-medium">{med.name}</p>
                                <p className="text-sm text-muted-foreground">
                                  {med.dosage} - {med.frequency}
                                </p>
                                {med.duration && <p className="text-sm text-muted-foreground">
                        ...
                        {prescription.valid_until && <p className="text-sm text-muted-foreground">
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <CardDescription className="mt-2 space-y-1">
                            ...
                            {appointment.doctors?.clinic_name && <p className="text-sm">{appointment.doctors.clinic_name}</p>}
                          </CardDescription>
                        </div>
